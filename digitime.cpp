...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//Ќа весы врем€ летит в таком формате: DDMMYY HHMM - 180517 1156
	//‘ормат команды сниффал Wireshark
	//f1 4e 00 00 00 ff 00       18 05 17 00 00 11 56 // так отправл€ет digiSync.exe на SM-100 не работает, SM-300 и SM-500 - ќ  -- Len 14
	...
	//ѕолучаем данные о весах: 55.	SCALE DATA FILE - (4FH)
	char cmd[] = {0xf7, 0x4f, 0x00, 0x00, 0x00, 0x00};
	retval = send_to_scale(cmd, sizeof(cmd), Buffer);
	if(retval>0){
		int model = Buffer[85];
		if(V) printf("Model: 0x%x\n", model);
		//ѕо весам которые были в наличии проверил
		// 0x11 - SM-100 -- дл€ этих нужен другой формат команды
		// 0x05 - SM-300
		// 0x01 - SM-500
		/* ј так написано в доке "FileStructure SM-101":
			* SCALE MAIN BOARD TYPE:
 				0 Ц SM90 TWB1600;				1 Ц SM90 TWB2150/2151;
				2 Ц SM500;						3 Ц SM90 TOUCH SCREEN;
 				4 Ц SM500 TOUCH SCREEN;			5Ц SM300
				90- DP90						91- DPS90
				92- DI90
				10- SM100						11- SM101(+)
		*/
		// ќтправл€ем команду весам и смотрим ответ 06,07- ок
